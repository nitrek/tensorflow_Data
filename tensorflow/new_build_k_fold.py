...
class DocReader():
    ...
    def input_data(self,publicDir,internalDir,restrictedDir, highlyRestrictedDir, percentTest,crossValidation,cutoff):
        ...
        if crossValidation == 0:
            ...
            numTest = int(.25 * len(pathLabelPairs))
            print numTest
            print len(pathLabelPairs)
            random.seed(100)
            set1 = set(random.sample(pathLabelPairs.items(),numTest))
            for entry in set1:
                del pathLabelPairs[entry[0]]
            random.seed(100)
            set2 = set(random.sample(pathLabelPairs.items(),numTest))
            for entry in set2:
                del pathLabelPairs[entry[0]]
            random.seed(100)
            set3 = set(random.sample(pathLabelPairs.items(),numTest))
            for entry in set3:
                del pathLabelPairs[entry[0]]
            random.seed(100)
            set4 = set(random.sample(pathLabelPairs.items(),numTest))
            for entry in set4:
                del pathLabelPairs[entry[0]]
            testing = set3
            training = set4
